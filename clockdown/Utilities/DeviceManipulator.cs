    ...
    internal class DeviceManipulator
    {
        ...
        public ICollection<MachineInfo> GetLogData(ZkemClient objZkeeper, int machineNumber)
        {
...

            {
                ...
                var anio = dwYear.ToString("D4");
                var mes = dwMonth.ToString("D2");
                var dia = dwDay.ToString("D2");
                var hora = dwHour.ToString("D2");
                var minuto = dwMinute.ToString("D2");
                var segundo = dwSecond.ToString("D2");
                var id = int.Parse(dwEnrollNumber1).ToString("D4");

                string key = string.Concat(id, anio, mes, dia, hora, minuto, segundo);

                MachineInfo objInfo = new MachineInfo();
                objInfo.KeyRecord = key;
        ...
        public ICollection<UserData> GetAllUserData(ZkemClient objZkeeper, int machineNumber)
        {
            string sdwEnrollNumber = null;
            string sName = null;
            string sPassword = null;
            int iPrivilege = 0;
            bool bEnabled = false;

            ICollection<UserData> lstData = new List<UserData>();
            ...
            while (objZkeeper.SSR_GetAllUserInfo(machineNumber, out sdwEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))
            {
                UserData fpData = new UserData();
                fpData.MachineNumber = machineNumber;
                fpData.EnrollNumber = sdwEnrollNumber;
                fpData.Name = sName;
                fpData.Password = sPassword;
                fpData.Privilege = iPrivilege;
                fpData.Enabled = bEnabled;
        ...
        //public bool UploadUserData(ZkemClient objZkeeper, int machineNumber)
        //{
        ...
        //    objZkeeper.SSR_SetUserInfo(machineNumber, dwEnrollNumber, Name, sPassword, iPrivilege, bEnabled);
        // }
